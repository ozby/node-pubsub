...
export const getQueueMetrics = async (req: Request<{ queueId: string }>, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const activeMessages = await Message.countDocuments({ queueId, received: false });
    const oldestMessage = await Message.findOne({ queueId, received: false }).sort({ createdAt: 1 });
